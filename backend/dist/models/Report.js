...
const reportDataSchema = new mongoose_1.Schema({
    summary: {
        totalQuestions: {
            type: Number,
...
reportSchema.methods.addNextStep = function (nextStep) {
    this.data.nextSteps.push(nextStep);
    return this.save();
};
// Method to get sections by type
reportSchema.methods.getSectionsByType = function (title) {
    return this.data.sections.filter(section => section.title.toLowerCase().includes(title.toLowerCase()));
};
...
reportSchema.statics.generateSummaryStats = async function (candidateId) {
    ...
    const totalScore = reports.reduce((sum, report) => sum + (report.data?.summary?.averageScore || 0), 0);
