...
interviewSchema.methods.calculateOverallScore = function () {
    if (!this.responses || this.responses.length === 0)
        return 0;
    const scoredResponses = this.responses.filter((response) => response.score !== undefined);
    if (scoredResponses.length === 0)
        return 0;
    const totalScore = scoredResponses.reduce((sum, response) => sum + response.score, 0);
...
interviewSchema.methods.getRemainingTime = function () {
    if (!this.startedAt)
        return this.settings?.timeLimit || 3600;
    const now = new Date();
