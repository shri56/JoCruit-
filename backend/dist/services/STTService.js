...
class STTService {
    ...
    async transcribeAudio(request) {
        try {
            const { audioBuffer, encoding = 'WEBM_OPUS', sampleRateHertz = 48000, languageCode = 'en-US' } = request;
            ...
            if (!response.results || response.results.length === 0) {
                logger_1.default.warn('No speech recognition results returned');
                return {
                    transcription: '',
                    confidence: 0,
                    words: [],
            ...
            logger_1.default.info('Speech-to-text transcription completed', {
            ...
            logger_1.default.error('Error in speech-to-text transcription:', error);
    ...
    async transcribeAudioFile(filePath, options = {}) {
        try {
            ...
            logger_1.default.info('Transcribed audio file', {
                filePath,
            ...
            logger_1.default.error('Error transcribing audio file:', error);
    ...
    async transcribeLongAudio(audioBuffer, options = {}) {
        try {
            const { encoding = 'WEBM_OPUS', sampleRateHertz = 48000, languageCode = 'en-US', storageUri } = options;
            let audioSource;
            if (storageUri) {
            ...
            if (!response.results || response.results.length === 0) {
                logger_1.default.warn('No long-running speech recognition results returned');
                return {
                    transcription: '',
                    confidence: 0,
                    segments: [],
            ...
            logger_1.default.info('Long-running speech-to-text completed', {
                transcriptionLength: transcription.length,
                averageConfidence,
            ...
            logger_1.default.error('Error in long-running speech transcription:', error);
    ...
    createStreamingRecognition(options = {}) {
        const { encoding = 'WEBM_OPUS', sampleRateHertz = 48000, languageCode = 'en-US', onTranscription, onError } = options;
        const recognizeStream = this.client
            ...
            if (data.results && data.results.length > 0) {
                ...
                if (alternative) {
                    ...
                    logger_1.default.debug('Streaming transcription result', {
                        transcription,
                        isFinal,
            ...
            .on('error', error => {
            logger_1.default.error('Streaming recognition error:', error);
            if (onError) {
                onError(error);
            }
        })
            .on('end', () => {
            logger_1.default.info('Streaming recognition ended');
        });
     ...
     */
    async batchTranscribe(audioFiles) {
        try {
            const results = await Promise.allSettled(audioFiles.map(async (file) => {
                try {
                    const result = await this.transcribeAudio({
            ...
            logger_1.default.info('Batch transcription completed', {
            ...
            logger_1.default.error('Error in batch transcription:', error);
     ...
     */
    getEncodingFromExtension(extension) {
        const encodingMap = {
            '.mp3': 'MP3',
            '.wav': 'LINEAR16',
            '.flac': 'FLAC',
    ...
    async analyzeAudioQuality(audioBuffer) {
        try {
            ...
            logger_1.default.info('Audio quality analysis completed', {
                fileSize,
                estimatedBitrate,
                quality,
                estimatedAccuracy
            ...
            logger_1.default.error('Error analyzing audio quality:', error);
            return {
                quality: 'fair',
